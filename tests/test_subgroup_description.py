...
class TestBasics(unittest.TestCase):
    def test_get_cover_array_and_size(self):
        ...
        self.assertEqual(
            ps.get_cover_array_and_size(np.array([1, 0, 1], dtype=bool))[1], 2
        )
    ...
    def test_get_size(self):
        ...
        self.assertEqual(
            ps.get_size(ps.EqualitySelector("checking_status", b"<0"), data=df), 274
        )
    ...
    def test_Conjunction(self):
        conj = ps.Conjunction(
            [ps.EqualitySelector.from_str("A==0"), ps.EqualitySelector.from_str("B==1")]
        )
    ...
    def test_create_numeric_selectors_for_attribute(self):
        ...
        selectors = ps.create_numeric_selectors_for_attribute(
            df, "A", nbins=4, intervals_only=False
        )
        result_selectors = interval_selectors_from_str(
            """A>=1
            A<1
            A>=3
            A<3
